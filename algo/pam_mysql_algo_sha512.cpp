/**
 * @file pam_mysql_algo_sha512.cpp
 *
...
private:
	 ...
	 */
	static unsigned char *pam_mysql_sha512_data(const unsigned char *d, unsigned int sz, unsigned char *md)
	{
		size_t i, j;
		unsigned char buf[64];

		if (!md) {
			md = (unsigned char *) xcalloc(128 + 1, sizeof(char));
            if (!md) {
				return NULL;
			}
		}

		SHA512(d, (unsigned long)sz, (unsigned char *) &buf);
...
public:
	pam_mysql_err_t check_password(const unsigned char *passwd, const char *dbpwd)
	{
        ...
        pam_mysql_sha512_data(passwd, strlen((const char *) passwd), (unsigned char *) &buf);
	...
	pam_mysql_err_t encryt_passwd(const char *new_passwd)
	{
		...
		pam_mysql_sha512_data((const unsigned char*) new_passwd, strlen(new_passwd), encrypted_passwd);
	}
};

const char PamMySQLAlgo_SHA512::OPTION_NAME[] = "SHA512";
